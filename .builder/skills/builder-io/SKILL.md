...
- In `app/preview/page.tsx`, pass `disableTracking` to `RenderBuilderContent` for the entry being edited; `disableTracking` sets `canTrack={false}`, which on `@builder.io/sdk-react@5.2.4+` already suppresses the A/B-test initializer script (no visible-text leak under React 19 SSR). Do NOT also pass `isNestedRender` on the root content being edited in `/preview` — it is an internal-only prop that also disables the `postMessage` listener the Builder visual editor needs for drag/drop, breaking in-editor editing.
- The `RenderBuilderContent` instance being edited on `/preview` must have no sibling elements in its immediate parent (mirror the bare `<RenderBuilderContent ... />` return used by `app/[...page]/page.tsx`). A sibling component (e.g. a decorative header) next to the editable `Content` on `/preview` can trigger a live-editing DOM reconciliation bug where the SDK's static personalization-init `<script>` renders as visible page text. Wrap only in a single static, non-reactive element if a `data-slot` hook is needed for tests — never place other rendered components alongside it.
